...
<form method="GET" action="{{ route('admin.doctors.index') }}">
    <div class="filter-bar">
        <input type="text" name="search" value="{{ request('search') }}" placeholder="Search by name…" style="flex:1;min-width:200px;"/>
        <select name="status">
...
@if($doctors->isEmpty())
    <div class="card empty-state">
        ...
        <div style="margin-top:16px;"><a href="{{ route('admin.doctors.create') }}" class="btn btn-green">Add First Doctor</a></div>
    </div>
@else
    <div class="table-wrap">
        <table>
            ...
            <tbody>
                ...
                <tr>
                    <td>
                        <div style="font-weight:700;">Dr. {{ $doctor->first_name }} {{ $doctor->last_name }}</div>
                    </td>
                    <td>{{ $doctor->department->name }}</td>
                    <td>{{ $doctor->experience_years }} yrs</td>
                    <td>⭐ {{ number_format($doctor->rating, 1) }}</td>
                    <td style="font-size:0.85rem;white-space:nowrap;">
                        {{ \Carbon\Carbon::parse($doctor->schedule_start)->format('h:i A') }}
                        – {{ \Carbon\Carbon::parse($doctor->schedule_end)->format('h:i A') }}
                    </td>
                    <td><span class="badge {{ $badgeClass }}">{{ $doctor->availability_status }}</span></td>
                    <td style="font-size:0.85rem;">
                        @if($doctor->user->email) <div>{{ $doctor->user->email }}</div> @endif
                        @if($doctor->user->phone) <div>{{ $doctor->user->phone }}</div> @endif
                    </td>
                    <td>
