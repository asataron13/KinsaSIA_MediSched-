...
<div style="display:grid;grid-template-columns:1fr 1fr;gap:22px;margin-bottom:22px;">
    ...
    {{-- Personal Info --}}
    <div class="card">
        <h3 style="margin-bottom:18px;font-size:1rem;">Personal Information</h3>
        <div class="info-grid">
            <div class="info-item">
            ...
            <div class="info-item">
                ...
                <div class="info-value">{{ \Carbon\Carbon::parse($patient->birth_date)->format('F d, Y') }} ({{ \Carbon\Carbon::parse($patient->birth_date)->age }} yrs)</div>
            </div>
            <div class="info-item">
                <div class="info-label">Status</div>
                <div class="info-value">
                    @php $s = strtolower($patient->status); @endphp
                    <span class="badge badge-{{ $s === 'active' ? 'active' : ($s === 'inactive' ? 'inactive' : 'pending') }}">{{ $patient->status }}</span>
                </div>
            </div>
    ...
    <div class="card">
        ...
        <div class="info-grid">
            ...
            <div class="info-item">
                ...
                <div class="info-value">{{ $patient->appointments->count() }}</div>
            </div>
        </div>
    </div>
</div>
...
<div class="card">
    <div class="card-header">
        ...
        <span style="font-size:0.85rem;color:#5a8a6e;">{{ $patient->appointments->count() }} total</span>
    </div>
    @if($patient->appointments->isEmpty())
        <p style="color:#5a8a6e;">No appointments on record.</p>
    @else
        <div class="table-wrap" style="box-shadow:none;">
            <table>
                <thead>
                    <tr><th>Date</th><th>Doctor</th><th>Department</th><th>Type</th><th>Status</th></tr>
                </thead>
                <tbody>
...
<div class="card">
    <div class="card-header">
        ...
        <a href="{{ route('admin.records.index') }}" class="btn btn-outline btn-sm">View All Records</a>
    </div>
    @if($patient->medicalRecords->isEmpty())
        <p style="color:#5a8a6e;">No medical records on file.</p>
    @else
        <div class="table-wrap" style="box-shadow:none;">
            <table>
                <thead>
                    <tr><th>Date</th><th>Type</th><th>Doctor</th><th>Diagnosis</th><th>Status</th></tr>
                </thead>
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>{{ $rec->diagnosis ?? '—' }}</td>
                        <td><span class="badge badge-{{ strtolower($rec->status) }}">{{ $rec->status }}</span></td>
                    </tr>
                    @endforeach
                </tbody>
            </table>
